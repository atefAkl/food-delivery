...
// إضافة بادئات admin و admin. للمسارات وفقاً لذاكرة المشروع
Route::prefix('admin')->name('admin.')->group(function () {
    // مسار للتحقق من حالة تسجيل الدخول - يمكن الوصول إليه سواء كان المستخدم مسجل أو غير مسجل
    Route::get('check-auth-status', function() {
        return [
            'isLoggedIn' => \Illuminate\Support\Facades\Auth::guard('admin')->check(),
            'user' => \Illuminate\Support\Facades\Auth::guard('admin')->user() ? ['email' => \Illuminate\Support\Facades\Auth::guard('admin')->user()->email, 'type' => \Illuminate\Support\Facades\Auth::guard('admin')->user()->type] : null
        ];
    });
    
    // مسارات للزوار غير المسجلين
    ...
    // جميع صفحات الادمن هنا محمية بالمصادقة فقط
