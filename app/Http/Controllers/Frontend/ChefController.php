...
class ChefController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // إحصائيات للشيف
        $totalDishes = $chef->dishes()->count();
        $totalOrders = $chef->orders()->count();
        $totalEarnings = $chef->orders()->sum('total_amount');
        $pendingOrders = $chef->orders()->whereIn('status', ['pending', 'processing'])->count();
        
        // آخر الطلبات
        $recentOrders = $chef->orders()
            ->with('customer.user')
